...
public class LSD {
    ...
    public static long charConsiderations = 0;
   ...
   /**
     ...
     * @param a the array to be sorted
     */
    public static void sort(int[] a, int base) {
        int bitsPerDigit = (int) (Math.log(base) / Math.log(2)) + 1;

        final int BITS = 32;                 // each int is 32 bits 
        final int R = 1 << bitsPerDigit;     // each bytes is between 0 and 255
        final int MASK = R - 1;              // 0xFF
        final int w = BITS / bitsPerDigit;   // each int is 4 bytes
